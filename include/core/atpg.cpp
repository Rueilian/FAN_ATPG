...
// **************************************************************************
// Function   [ Atpg::precomputeImplications ]
// Synopsis   [ Precompute static (SOCRATES-style) implications: for each
//              gate, record which of its immediate fanout gates get forced
//              when this gate takes a controlling value.
//            ]
// **************************************************************************
void Atpg::precomputeImplications()
{
	if (!useStaticLearning_) return;
	learnedFrom_[0].clear();
	learnedFrom_[1].clear();
	learnedFrom_[0].resize(pCircuit_->totalGate_);
	learnedFrom_[1].resize(pCircuit_->totalGate_);
	for (int g = 0; g < pCircuit_->totalGate_; ++g)
	{
		Gate &gate = pCircuit_->circuitGates_[g];
		for (int fo = 0; fo < gate.numFO_; ++fo)
		{
			int fId = gate.fanoutVector_[fo];
			Gate &fanout = pCircuit_->circuitGates_[fId];
			switch (fanout.gateType_)
			{
				case Gate::AND2: case Gate::AND3: case Gate::AND4:
					learnedFrom_[L][g].push_back({fId, L});
					break;
				case Gate::NAND2: case Gate::NAND3: case Gate::NAND4:
					learnedFrom_[L][g].push_back({fId, H});
					break;
				case Gate::OR2: case Gate::OR3: case Gate::OR4:
					learnedFrom_[H][g].push_back({fId, H});
					break;
				case Gate::NOR2: case Gate::NOR3: case Gate::NOR4:
					learnedFrom_[H][g].push_back({fId, L});
					break;
				case Gate::BUF:
					learnedFrom_[L][g].push_back({fId, L});
					learnedFrom_[H][g].push_back({fId, H});
...
// **************************************************************************
// Function   [ Atpg::checkStaticConflict ]
// Synopsis   [ After gate `gateId` is assigned value `val`, check if any
//              learned implication contradicts an already-assigned value.
//              Returns true if conflict is found.
//            ]
// **************************************************************************
bool Atpg::checkStaticConflict(int gateId, Value val) const
{
	if (val != L && val != H) return false; // only binary values have implications
	for (const auto &entry : learnedFrom_[val][gateId])
	{
		Value current = pCircuit_->circuitGates_[entry.first].atpgVal_;
		if (current != X && current != entry.second)
			return true;
	}
	return false;
}
...
Atpg::IMPLICATION_STATUS Atpg::evaluateAndSetGateAtpgVal(Gate *pGate)
{
	...
	if (pGate->atpgVal_ == X)
	{
		...
		pGate->atpgVal_ = Val;
		if (useStaticLearning_ && Val != X && checkStaticConflict(pGate->gateId_, Val))
			return CONFLICT;
		backtrackImplicatedGateIDs_.push_back(pGate->gateId_);
		gateToDecisionLevel_[pGate->gateId_] = currentDecisionLevel_;
		gateID_to_valModified_[pGate->gateId_] = 1;
	...
	if (useNineValuedLogic_)
	{
		...
		if (reconcileGateAtpgVal(pGate->atpgVal_, Val))
		{
			...
			{
				if (useStaticLearning_ && pGate->atpgVal_ != X &&
				    checkStaticConflict(pGate->gateId_, pGate->atpgVal_))
					return CONFLICT;
				backtrackImplicatedGateIDs_.push_back(pGate->gateId_);
				gateToDecisionLevel_[pGate->gateId_] = currentDecisionLevel_;
				pushGateFanoutsToEventStack(pGate->gateId_);
